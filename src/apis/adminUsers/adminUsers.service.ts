...
export class AdminUsersService {
  ...
  setAdminRefreshToken({ adminUser, res, req }) {
    const refreshToken = this.jwtService.sign(
      ...
      { secret: process.env.JWT_ADMIN_REFRESH_SECRET, expiresIn: '2w' },
    );
  ...
  getAdminAccessToken({ adminUser }) {
    return this.jwtService.sign(
      { account: adminUser.account, sub: adminUser.id },
      { secret: process.env.JWT_ADMIN_ACCESS_SECRET, expiresIn: '1h' },
    );
  }
   ...
   */
  async create(createAdminUserInput) {
    const admin = await this.adminUsersRepository.findOne({
      where: { account: createAdminUserInput.account },
    });
    if (admin) throw new ConflictException('이미 등록된 관리자 계정입니다.');
    ...
    const result = this.adminUsersRepository.save({
